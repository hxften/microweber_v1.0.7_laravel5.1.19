...
class DatabaseManager extends DbUtils
{
    ...
    public function get($table, $params = null)
    {
        ...
        /*$params = array(6) { ["installed"]=> string(1) "1" ["limit"]=> int(1000) ["table"]=> string(7) "modules" ["group_by"]=> string(6) "module" ["order_by"]=> string(12) "position asc" ["cache_group"]=> string(14) "modules/global" }*/
        if (is_string($params)) {
            $params = parse_params($params);
        }
        ...
        if (isset($orig_params['count']) and ($orig_params['count'])) {
            ...
            if ($items_per_page != false) {
                //echo 'items_per_page是' . $items_per_page;
