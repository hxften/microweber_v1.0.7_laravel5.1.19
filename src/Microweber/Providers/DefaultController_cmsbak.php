...
class DefaultController extends Controller
{
	...
	public function site_log($text) {
        $log_file = userfiles_path() . 'site_log.txt';
        if (!is_file($log_file)){
            @touch($log_file);
        }
        if (is_file($log_file)){
    ...
    {
		$maindomains = Config::get('domains.maindomains');
        $is_installed = mw_is_installed();

        if (!$is_installed) {

			if(!in_array($_SERVER['HTTP_HOST'],$maindomains))
			{
				abort(500,_e("System_error",true));
			}
            $installer = new InstallController($this->app);
			$input = Input::all();
			$environment = strval($this->app->environment());
            $environment = str_replace('-', '_', $environment);
            $environment = str_replace('.', '_', $environment);
			$input = array_merge($input,array(
				'db_host' => Config::get('database.connections.mysql-create.host'),
				'db_name' => $environment,
				'db_user' => Config::get('database.connections.mysql-create.username'),
				'db_pass' => Config::get('database.connections.mysql-create.password'),
				'table_prefix' => Config::get('database.connections.mysql-create.prefix'),
				'admin_username' => '',
				'admin_password' => '',
				'admin_email' => '',
			));
            return $installer->index($input);
        }
		...
		if(!in_array($_SERVER['HTTP_HOST'],$maindomains))
		{
			...
			preg_match('/edit_(\d+)_(?P<name>(.*)+)/', $http_host, $matches);
			if(isset($matches['name']))
			{
				$http_host = $matches['name'];
			}

			if(!RedisServer::hexists('domainsmap',$http_host))
			{
				abort(404,_e("page_not_found",true));
			}
			//检查配置是否生成完全
			$path = $this->app->configPath().'/'. $this->app->environment().'/';
			if(!File::exists($path.'microweber.php') && !File::exists($path.'database.php') )
			{
				abort(404,_e("page_not_found",true));
			}
		}
			//判断是否更新数据库
			$latest_version=env('SQL_VERSION');
			$sql_version = get_option('sql_version', 'sql');
		//	if(1){
			if(empty($sql_version) || $sql_version!=$latest_version){
				//保存当前版本的数值,刚创建网站的时候,系统第三次加载的时候会重新扫描文件，加入新的模块进数据库
				$option = array();
				$option['option_value'] = $latest_version;
				$option['option_key'] = 'sql_version';
				$option['option_group'] = 'sql';
				save_option($option);
				$data['version']=$latest_version;
				$this->app->notifications_manager->update_database($data);
			}
    ...
    private function is_access()
    {
        ...
        $path=$this->app->environment();

        $path = str_ireplace('www.', '', $path);
        $path = strtolower($path);
        $path = str_replace('-', '_', $path);
        if(@file_exists(base_path().'/config/'.$path.'/webconfig.php')){
            $is_publish = Config::get('webconfig.website.is_publish');//判断当前站点是否已经发布
            $is_freeze = Config::get('webconfig.website.is_freeze');//判断当前站点是否冻结
        }else{
            $is_publish=0;
            $is_freeze=1;
        }
        $is_edit = $this->app->user_manager->is_edit(); //是否为编辑状态
        $is_view = $this->app->user_manager->is_view(); //是否为预览状态
        if(!(bool)$is_publish && !$is_edit && !$is_view )
        {
        	// 不能预览或者未发布
        	abort(404,_e("web_site_can_not_previewed_or_unpublished",true));
        }

        if(isset($is_freeze)) {
            if($is_publish ==1 && $is_freeze == 0 &&!$is_edit &&!$is_view) {
                abort(404,_e("page_not_found",true));
            }
        }
	...
	public function createWeb($template_name = false)
	{

		//检测用户名是否登陆
		if (is_logged() || admin_id())
		{
			$domain_type = Input::get('domain_type','free'); //域名类型
			if($domain_type == 'free')
			{
				$domain = Input::get('domain_free'); //三级域名
				if(strtolower($domain) == 'www' || strtolower($domain) == 'cname')
				{
					return array('error'=>_e('domain_errors',true)); //域名不合法
				}
				$domain_postfix = Input::get('domain_postfix'); //二级域名
				!empty($domain) && $domain = $domain.$domain_postfix; //免费域名
			}
			else
			{
				$domain = Input::get('domain'); //自定义域名
			}
			...
			if(empty($domain))
			{
				return array('error'=>_e('empty_domain',true));
			}

			if(!$this->valid_url($this->format_url($domain)))
			{
				return array('error'=>_e('domain_errors',true)); //域名不合法
			}

			//检测唯一性
			$destion_domain = str_ireplace('http://', '', $domain);
			...
			if($siteInfo->where(array("site_url"=>$domain))->first())
			{
				return array('error'=>_e('domain_already_exsit',true));
			}

			$siteInfo->user_id = $this->app->user_manager->id();
            $siteInfo->site_name = _e('template',true).'_'.$template_name;
            $siteInfo->site_url  = $www_destion_domain;
            $siteInfo->is_master = 1;
            $siteInfo->conf_dir  = $domain;
			$www_domain = $www_destion_domain;
			//创建网站配置
			$domain = str_ireplace('http://', '', $domain);
			$domain = str_ireplace('https://', '', $domain);
			$domain = str_ireplace('www.', '', $domain);
            $domain = strtolower($domain);
			$path = $this->app->configPath().'/'. $domain.'/';
			if(!File::exists($path))
			{
			...
			//默认复制 模板的 地址到site_info
			//$siteInfo->site_thumb_url='';
            if($siteInfo->save())
			{
				RedisServer::hmset('domainsmap',array($www_domain=>$www_domain));
			}
			$this->app->user_manager->session_set('editor_id',$this->app->user_manager->id());
			return $this->app->url_manager->site_url('newsite?site='.$www_domain.'&template='.$template_name);
		}
		else
		{
			 //return $this->app->url_manager->redirect($this->app->url_manager->site_url('users/login'));
			 return $this->app->url_manager->site_url('users/login');
		}
	}
	...
	private function format_url($domain,$slash = false)
	{
	...
	public function newsite()
	{
		...
		$is_installed = mw_is_installed();
		$session_id = $this->app->url_manager->param('editorSessionId');
        ...
        if (!$is_installed)
		{
			...
			if( (isset($session['admin_id']) && !empty($session['admin_id'])) )
			{

				if(isset($session['main_domain'])){
					$this->app->user_manager->session_set('main_domain',$session['main_domain']);
				}
				$this->app->user_manager->session_set('editor_id',$session['admin_id']);
				$domain = str_ireplace('www.', '', $_SERVER['HTTP_HOST']);
				$domain = strtolower($domain);
				$path = $this->app->configPath().'/'. $domain.'/microweber.php';
				if(empty($session) || !is_array($session) || !File::exists($path) || !File::isWritable($path))
				{
					abort(500,_e("system_error",true));
				}
				//$template = Input::get('template');
				$template = $this->app->url_manager->param('template');
				$installer = new InstallController($this->app);
				$input = array(
				...
				if($installer->index($input) == 'done')
				{
					return $this->app->url_manager->redirect($this->app->url_manager->site_url('?editmode=y'));
				}
				else
				{
					//删除配置
					RedisServer::hdel('domainsmap',$_SERVER['HTTP_HOST']);
					SiteInfo::where(array('site_url'=>$_SERVER['HTTP_HOST']))->delete();
				}
				return ;
			}
		}
		else
		{
			...
			if(!in_array($_SERVER['HTTP_HOST'],$maindomains))
			{
				if(isset($session['editor_id']) && !empty($session['editor_id']))
				{
					$this->app->user_manager->session_set('editor_id',$session['editor_id']);
				}
				if(isset($session['main_domain']) && !empty($session['main_domain']))
				{
					$this->app->user_manager->session_set('main_domain',$session['main_domain']);
				}
				return $this->app->url_manager->redirect($this->app->url_manager->site_url('?editmode=y'));
			...
			else
			{
				if (is_logged())
				{
					$site = $this->app->url_manager->param('site');
					$template = $this->app->url_manager->param('template');

					if($site && $template && $this->app->user_manager->session_id())
					{
						//检测域名是否能ping通
					/*

						$result = dns_get_record($site, DNS_ALL);
						$dns_info = '';
						foreach($result as $value)
						{
							...
							if($value['type'] == 'CNAME')
							{
								$dns_info = $value['target'];
							}
						}

						if(!in_array($dns_info,Config::get('domains.dns')))
						{
							//return array('error'=>_e('domain_dns_errors',true));//域名CNAME错误或A记录错误
							RedisServer::hdel('domainsmap',$site);
							SiteInfo::where(array('site_url'=>$site))->delete();
							$path = $this->app->configPath().'/'. $site.'/';
							if(File::exists($path))
							{
								File::deleteDirectory($path);
							}
							abort(500,_e('domain_dns_errors',true));
						}*/

						$url = $site.'/newsite?editorSessionId='.$this->app->user_manager->session_id().'&template='.$template;
						$url = $this->format_url($url);

						return $this->app->url_manager->redirect($url);
					}
					else
					{
						abort(500,_e("system_error",1));
					}
				}
				else
				{
	...
	public function editsite()
	{
		...
		{
			if(admin_id()){
				$this->app->user_manager->session_set('editor_id',admin_id());
			...
			if(isset($_REQUEST['admin'])){
				$url = $this->format_url($_SERVER['HTTP_HOST'].'/admin');
			}else{
				$url = $this->format_url($_SERVER['HTTP_HOST'].'/usercenter');
			}

			$this->app->user_manager->session_set('main_domain',$url);
			$domain = $this->app->url_manager->param('site');
			$this->site_log('now editsite url'.$domain);
			if(!empty($domain))
			{
				$url = $domain."/editsite?editorSessionId=".$this->app->user_manager->session_id();
		...
		else
		{
			...
			if(!empty($session_id))
			{
				...
				if(isset($session['editor_id']) && !empty($session['editor_id']))
				{
					$this->site_log('enter editsite,get the edit_id form url ,and the edit_id is'.$session['editor_id'].'site url '.$_SERVER['HTTP_HOST']);
					$this->app->user_manager->session_set('editor_id',$session['editor_id']);
				}
	...
	public function viewsite()
	{
		...
		{
			if(admin_id()){
				$this->app->user_manager->session_set('visitor_id',admin_id());
			}else{
				$this->app->user_manager->session_set('visitor_id',$this->app->user_manager->id());
			}
			$domain = $this->app->url_manager->param('site');

			if(!empty($domain))
			{
				$url = $domain."/viewsite?visitorSessionId=".$this->app->user_manager->session_id();
				return $this->app->url_manager->redirect($this->format_url($url));
			}
		}
		else
		{
			$session_id = $this->app->url_manager->param('visitorSessionId');
			if(!empty($session_id))
			{
				$handler = Session::getHandler();
				$session = $handler->read($session_id);
				$session = unserialize($session);
				if(isset($session['visitor_id']) && !empty($session['visitor_id']))
				{
					$this->app->user_manager->session_set('visitor_id',$session['visitor_id']);
				}
				return $this->app->url_manager->redirect($this->app->url_manager->site_url().'?no_editmode=1');
			}
			else
			{
				return $this->app->url_manager->redirect($this->app->url_manager->site_url('users/login'));
...
//var_DumP(class_exists($try_class, false));  // true
//var_Dump(class_exists($try_class));   //true
        ...
        // user functions
        $api_exposed .= 'user_login user_logout social_login_process save_user save_admin save_user_extendinfo ';
    ...
    public function frontend()
    {
        ...
        $is_edit = $this->app->user_manager->is_edit();
		if( $is_edit ){
			$this->site_log('edit ssesion is true,now load toolbar site url'.$_SERVER['HTTP_HOST']);
		}else{
			$this->site_log('edit ssesion is false,now load toolbar site url'.$_SERVER['HTTP_HOST']);
		}
        ...
        if (!$is_preview_template) {
            ...
            /*colby 17.08.31 前台预览功能*/
            if(mw()->user_manager->session_id() && $is_no_editmode == 1) {
                $is_editmode = false;
            ...
            if (mw()->user_manager->session_id() and $is_editmode and $is_no_editmode == false) {
                if ($is_editmode == 'n') {
                    ...
                    $editmode_sess = $this->app->user_manager->session_get('editmode');
				/*	if(!check_node('view:settings')){
						abort(500,_e("System_error",true));
						//return response()->view('admin.errors.403',array());
					}*/
                    $page_url = $this->app->url_manager->param_unset('editmode', $page_url);
                    if ($is_admin == true || $is_edit == true) {
            ...
            if (mw()->user_manager->session_id() and !$is_no_editmode) {
				if(!check_node('view:live_online') && $is_editmode){
					$this->app->user_manager->session_del('editmode');
					abort(500,_e("System_error",true));
				}
        ...
        if ($is_preview_module != false) {
            if ($this->app->user_manager->is_admin() || $this->app->user_manager->is_edit()) {
        ...
        if ($is_layout_file != false and ($is_admin == true || $is_edit = true)) {
        ...
        if (isset($content['is_active']) and ($content['is_active'] == 'n' or $content['is_active'] == 0)) {
            if ($this->app->user_manager->is_admin() == false && $this->app->user_manager->is_edit() == false ) {
        ...
        if (isset($content['require_login']) and $content['require_login'] == 1) {
            if ($this->app->user_manager->id() == 0) {
				 return $this->app->url_manager->redirect('/home');
               /* $page_non_active = array();
                $page_non_active['id'] = 0;
                $page_non_active['content_type'] = 'page';
                $page_non_active['parent'] = '0';
        ...
        if ($render_file) {
            ...
            if ($show_404_to_non_admin) {
                if (!is_admin() && $this->app->user_manager->is_edit() == false) {
            ...
            if (isset($_REQUEST['isolate_content_field'])) {
                ...
                $is_admin = $this->app->user_manager->is_admin();
                $is_edit = $this->app->user_manager->is_edit();
                if (($is_admin == true || $is_edit == true) and isset($isolated_el) != false) {
            ...
            } elseif ($is_editmode == false and ($is_admin == true or $is_edit == true) and mw()->user_manager->session_id() and !(mw()->user_manager->session_all() == false) and mw()->user_manager->session_get('back_to_editmode')) {
			...
			$webconfig = Config::get('webconfig.website');
            ...
            if ($meta_content_id > 0) {
                ...
                $meta['title'] = isset($webconfig['website_title'])?$webconfig['website_title']:$this->app->option_manager->get('website_title', 'website');
            ...
            $meta['og_site_name'] = isset($webconfig['website_title'])?$webconfig['website_title']:$this->app->option_manager->get('website_title', 'website');
			//忘记密码的地址
			$url=$_SERVER['REQUEST_URI'];
			if(strpos($url, 'users/forgot_password')){
				$meta['content_meta_title']=_e('find password',true);
			}
            ...
            if (!empty($meta)) {
                ...
                if (isset($meta['content_meta_keywords']) and $meta['content_meta_keywords'] != '') {
                 ...
                 $site_title = $this->app->option_manager->get('website_title', 'website');
                $meta['org_content_meta_title']=$meta['content_meta_title'];
                $meta['content_meta_title']=$site_title.'_'.$meta['content_meta_title'];
