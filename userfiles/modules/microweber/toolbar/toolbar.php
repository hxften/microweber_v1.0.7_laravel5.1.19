...
<div class="mw-defaults" id="live_edit_toolbar_holder" dir="ltr" lang="en">
        ...
        <ul>
          ...
          <li class="create-content-dropdown">  
            ...
            <?php if(mw()->ui->logo_live_edit != false) : ?>
               <span style="background-image:url('<?php print mw()->ui->logo_live_edit ?>'); width:30px; height:30px; display:inline-block; background-size:contain; background-repeat:no-repeat; background-position:center; position:relative; top:8px;">
               </span>
    				<?php else: ?>
                <!--Microweber 显示-->
            ...
            <div class="mw-dropdown-list create-content-dropdown-list">
              ...
              <div class="mw-dropdown-list-search">
                <input type="mwautocomplete" class="mwtb-search mw-dropdown-search mw-ui-searchfield" placeholder="Search content"/>
                  ...
                  ?>
              <a id="backtoadminindropdown" class="mw-ui-btn mw-ui-btn-invert" href="<?php print $back_url; ?>" title="<?php _e("Back to Admin"); ?>"> <span class="mw-icon-back"></span><span><?php _e("Back to Admin"); ?></span> </a> 
            </div>
          </li>
          ...
          <li class="create-content-dropdown mw-toolbar-btn-menu">
            <!-- 新建 -->
            <a href="javascript:;" class="mw-ui-btn mw-ui-btn-medium mw-dropdown-button mw-toolbar-add-new-content-ctrl" title="Create or manage your content" style="">
            <span> <?php _e("Add New"); ?> 1</span>
            </a>
  ...
  array(3) {
    ...
    <!--mw-icon-page mw-icon-post mw-icon-category mw-icon-product -->
    <?php $class = ( isset( $item['class']))? ($item['class']) : false ; ?>
    <?php $html = ( isset( $item['html']))? ($item['html']) : false ; ?>
    <!--page post category product-->
    <?php $type = ( isset( $item['content_type']))? ($item['content_type']) : false ; ?>
    <?php $subtype = ( isset( $item['subtype']))? ($item['subtype']) : false ; ?>
    
    <li>
       <a onclick="mw.quick.edit('0','<?php print $type; ?>', '<?php print $subtype; ?>', '<?php print MAIN_PAGE_ID; ?>', '<?php print CATEGORY_ID; ?>'); return false;" href="<?php print admin_url('view:content'); ?>#action=new:<?php print $type; ?>
       <?php if($subtype != false): ?>.<?php print $subtype; ?><?php endif; ?>">
