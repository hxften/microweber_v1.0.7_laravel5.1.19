...
mw.edit_content.handle_form_submit = function (go_live) {
    ...
    //[div#mw-quick-page.module.module-content-edit]
    ...
    mw.content.save(data, {
        onSuccess: function (a) {// 走
            mw.$('.mw-admin-go-live-now-btn').attr('content-id', this);
            mw.askusertostay = false;

            if (parent !== self && !!window.parent.mw) {// 走
                window.parent.mw.askusertostay = false;
                if (typeof(data.is_active) !== 'undefined' && typeof(data.id) !== 'undefined') {// 走
                    if ((data.id) != 0) {
                        if ((data.is_active) == 0) {
